...
- **[Python API](docs/PYTHON_API.md)** — for embedders. The
  `LitterboxAgent` class methods documented one by one for programmatic use.
- **[Developer Intro](docs/DEVELOPER_INTRO.md)** — for engineers extending
  the codebase but without an ML background. Architecture, file map,
  common tasks, things that bite.
- **[ML Tutorial](docs/ML_TUTORIAL.md)** — for the technically curious.
  Each algorithm in the system explained from first principles: CLIP
  cosine similarity, robust median+MAD, log-Gaussian fits, PCA,
  GMM+BIC, Welch-style mean-shift z-tests.
- **[Testing](docs/TESTING.md)** — test architecture, how to run the
  suite, baseline results.
- **`simulator/oc_report.md`** — generated by `simulator/oc_study.py`.
  Operating-characteristic study with FPR/TPR for every detector at the
