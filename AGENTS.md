...
- **Litter Box Monitor** (`src/litterbox_agent.py`) — Automated cat health monitoring via litter box camera images, with 16 LangChain tools, two-stage CLIP+GPT-4o identity pipeline, sensor data ingestion (weight scale, gas sensors), and health analysis
...
- **`src/litterbox/trend_anomaly.py`** — Long-term per-cat trend detector. Fills the gap left by the per-visit detectors (`gas_anomaly`, `eigen`, `cluster`), which are deliberately blind to slow drifts. `score_trends(conn, cat_id, *, days_recent=14, days_baseline=75)` splits the cat's history into a recent window (last 14 days) and the baseline (the 75 days before that), and reports a tier ∈ {`normal`, `mild`≥2σ, `significant`≥3σ, `severe`≥5σ, `insufficient_data`} per channel for `cat_weight_g`, `waste_weight_g`, `ammonia_peak_ppb`, `methane_peak_ppb`. The mean-shift z-score uses Welch-style standard error with MAD-based σ. Direction-aware overlays: weight gets an additional clinical % overlay (5/10/15% of body weight, takes worse of z-tier or pct-tier); NH₃/CH₄ alarm only on the high side; waste alarms only under a constipation rule (no-waste-rate ≥ 50%, ratio to baseline ≥ 2×, AND mean waste z-score ≤ -2). When the constipation flag fires, the waste channel is escalated to at least `significant`. Pure function, no schema additions. Min sample gates (`min_visits_recent=5`, `min_visits_baseline=10`) prevent verdicts on under-sampled cats. Calibrated against synthetic data in `simulator/oc_study.py` — at the documented thresholds the per-channel ≥mild FPR is 5–14% (matching 2σ Gaussian tail), overall ≥significant FPR ≈ 7.5%, ≥severe FPR < 1%; sensitivity is high (3% body-weight drop hits ≥significant 98% of the time, 50%-no-waste pattern fires the constipation rule 39% of the time, 70%-no-waste fires it 84% of the time).
- **`src/litterbox/tools.py`** — 16 `@tool` functions; docstrings auto-generate LangChain tool descriptions. Per-visit detail (`get_visit_details(visit_id)`), history-plot generation (`plot_cat_history(cat_name, days=90)`), the eigenanalysis HTML report (`eigen_report(cat_name)`, added with Step 5b), the long-term trend report for one cat (`get_trend_summary(cat_name)`), and the cross-cat trend-alarm scan (`get_trending_cats()`) supplement the existing list-style query tools. Trend alarms also auto-surface at the bottom of `get_anomalous_visits` so the agent does not have to explicitly call the trend tool to notice active alarms — the scan is computed fresh on each call from `_scan_trend_alarms()`, no schema, no cache, no daily roll-up needed.
