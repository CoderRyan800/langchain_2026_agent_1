...
| `noncat.jpg` | Generated by Pillow (solid blue rectangle) | Manual-runner below-threshold identification check |
...
- **Manual-runner non-cat similarity: 0.5192** — a solid blue rectangle scores 0.52 against a cat photo in the manual runner, comfortably below the 0.82 threshold. The slow pytest CLIP suite uses real simulator cat photos for the different-cat threshold test, because solid-color images are not a meaningful proxy for cat identity separation.
...
| `tests/test_trend_anomaly.py` | — | Long-term per-cat trend detector + auto-trigger surface | 47 |
| **Subtotal** | | | **348** |
...
`test_api.py`) currently runs **650 non-slow tests** in about 52 s. Add
