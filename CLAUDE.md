...
- **Litter Box Monitor** (`src/litterbox_agent.py`) — Automated cat health monitoring via litter box camera images, with 15 LangChain tools, two-stage CLIP+GPT-4o identity pipeline, sensor data ingestion (weight scale, gas sensors), and health analysis
...
- **`src/litterbox/tools.py`** — 15 `@tool` functions; docstrings auto-generate LangChain tool descriptions. Per-visit detail (`get_visit_details(visit_id)`), history-plot generation (`plot_cat_history(cat_name, days=90)`), the eigenanalysis HTML report (`eigen_report(cat_name)`, added with Step 5b), and the long-term trend report (`get_trend_summary(cat_name, days_recent=14, days_baseline=75)`) supplement the existing list-style query tools so the agent can answer specific "explain visit N", "show me a chart", "show me the eigenanalysis report", and "is Luna trending differently lately" questions.
